...
def test_gold_creates_db_and_tables(sample_silver_parquet, tmp_path):
    ...
    summary = con.execute("SELECT locality_name, case_count, avg_fine FROM monthly_locality_summary").fetchone()
    assert summary[0] == "Albemarle"
    assert summary[1] == 2
    assert summary[2] == 100.0  # (150 + 50) / 2
    
    # Verify efficiency calculation
    efficiency = con.execute("SELECT avg_days_to_hearing FROM locality_efficiency WHERE locality_name = 'Albemarle'").fetchone()[0]
