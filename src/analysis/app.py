...
st.set_page_config(
    page_title="Court Cases Analysis",
    page_icon="⚖️",
    layout="wide"
)
...
def get_connection():
    ...
    if not os.path.exists(DB_PATH):
        st.error(f"Database not found at {DB_PATH}. Please run the data loader script first.")
        return None
    try:
...
def main():
    ...
    if page == "SQL Editor":
        ...
        try:
            ...
            # Dismissal Rate
            dismissed = conn.execute("SELECT COUNT(*) FROM court_cases WHERE dispositionText = 'D'").fetchone()[0]
