...
def load_and_preprocess(file_path):
    ...
    df["dispositionText"] = df["disposition"].apply(
        lambda x: (
            x.get("dispositionInfo", {}).get("dispositionText")
            if isinstance(x, dict)
            else None
        )
    )
    df["dispositionDate"] = pd.to_datetime(
        df["disposition"].apply(
            lambda x: (
                x.get("dispositionInfo", {}).get("dispositionDate")
                if isinstance(x, dict)
                else None
            )
        ),
    ...
    def extract_part_details(p):
        ...
        if isinstance(attorneys, list):
            for att in attorneys:
                b_name = (
                    att.get("attorneyBusinessName", {}).get("businessName", "").upper()
                )
    ...
    def get_fin(row):
        try:
            ...
            fines = float(
                fin.get("fines", {}).get("imposedAmount", {}).get("decimal", 0.0)
            )
